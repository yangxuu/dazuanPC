...
export default {
  ...
  created() {
    this.type = localStorage.getItem("color_type");
  },
  mounted() {
    document.getElementById("app").className = "theme" + this.type;
  }
};
</script>
<style lang='less'>
...
.themea {
  .theme(@backcolor:purple);
}
.themeb {
  .theme(@backcolor:green);
}
.themec {
  .theme(@backcolor:pink);
}
</style>
